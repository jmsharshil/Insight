"""
Data migration: Populate subjects and chapters for CRS-0007 (Company Secretary)
────────────────────────────────────────────────────────────────────────────────
Creates subjects and chapters under the CSEET, CS Executive, and CS Professional
course levels. Uses get_or_create to be idempotent (safe to re-run).

Source: "CHAPTER, CLASS TEST, PRACTICE SESSION & PRELIM DONE.xlsx"
"""
...
# ═══════════════════════════════════════════════════════════════════════════════
#  Chapter data extracted from the xlsx file
# ═══════════════════════════════════════════════════════════════════════════════
...
COURSE_CODE = "CRS-0007"


def populate_chapters(apps, schema_editor):
    """Create subjects and chapters from the hardcoded data."""
    ...
    try:
        ...
        print(f"\n  ⚠️  Course {COURSE_CODE} not found — skipping chapter population.")
        return

    for level_name, subjects_data in CHAPTER_DATA.items():
        try:
            level = CourseLevel.objects.get(course=course, name=level_name)
        except CourseLevel.DoesNotExist:
            print(f"\n  ⚠️  CourseLevel '{level_name}' not found under {COURSE_CODE} — skipping.")
            continue
        ...
        for subj_data in subjects_data:
            ...
            for order, ch_name in enumerate(subj_data["chapters"], start=1):
                ...
                if ch_created:
                    print(f"    📖 Ch {order}: {ch_name}")


def reverse_chapters(apps, schema_editor):
    """Remove all subjects/chapters created by this migration under CRS-0007."""
    Course = apps.get_model("batches", "Course")
    CourseLevel = apps.get_model("batches", "CourseLevel")
    Subject = apps.get_model("batches", "Subject")
    ...
    for level_name, subjects_data in CHAPTER_DATA.items():
        ...
        Subject.objects.filter(level=level, name__in=subject_names).delete()
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(populate_chapters, reverse_chapters),
    ]
